  ...
  return (
    <main className="max-w-xl min-w-md mx-auto mt-6">
      <div className="my-5 flex flex-col items-start flex-nowrap">
        <input className="border rounded border-slate-600 px-2 w-full bg-black" value={userId} onChange={(e) => {if(e.target.value!=''){setUserId(e.target.value)}}} placeholder="Enter user ID" />
        <button className="border rounded mt-2 border-slate-600 px-2 w-full" onClick={() => setUserId(userId)}>Load Messages</button>
      </div>

      {userProfile ? (
        <div className="my-5">
          <Image src={userProfile.pictureUrl} alt={userProfile.displayName} width="300" height="300" className="rounded-full" />
